...
Where $W^{(1)}$ is a 4x3 matrix and $b^{(1)}$ is a vector of length 3, both of which have values that are learned during training. A common activation function is the **Rectifier Linear Unit (ReLU)** which simply has the form:

$$
\text{ReLU}(x) = \text{max}(0,x)
$$ {eq-relu}

Although @eq-sigmoid is also a common activation function due to its controlled range of (0,1). 
