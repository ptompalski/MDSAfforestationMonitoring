...
The tables highlight the F1 Scores for LSTM and GRU models using Site + Satellite and Satellite features at 50% and 80% thresholds. At the 50% threshold, all models and feature combinations show an F1 Score of 0, indicating no predictive capability. At the 80% threshold, performance improves: LSTM with Site + Satellite features, as shown in @tbl-lstm-site-scores, achieves an F1 Score of 0.368, while LSTM with Satellite features, as shown in @tbl-lstm-no-site-scores, reaches 0.393. GRU with Satellite features, as shown in @tbl-tbl-gru-no-site-scores, records a higher F1 Score of 0.434, and GRU with Site + Satellite features, as showin in @tbl-gru-site-scores, attains 0.44, demonstrating the best performance at this threshold. While GRU consistently outperforms LSTM, with notable improvements at the 80% threshold, it fails to outperform our classical machine learning models.
